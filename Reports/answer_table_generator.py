# This module contains function that generates table of answers for the report pdf.
# Solution by https://github.com/bvalgard/create-pdf-with-python-fpdf2/blob/main/create_table_fpdf2.py
def formate_data_for_answer_table(all_answers):
    """

    :param all_answers: all answers of the current score
    :return: formatted data which are ready to be printed as table with "create_table()"
    """
    data = [["ABSOLUTE TIME [s]", "QUESTION", "ANSWER", "RESULT", "REACTION TIME [ms]"]]
    convert_answers = {"g": "green", "r": "red", "b": "blue", "w": "white", "y": "yellow", "up": "high tone",
                       "down": "low tone", "right": "right pedal", "left": "left pedal"}

    # Iterate through answers and formate each answer
    for answer in all_answers:
        answer = list(map(str, answer))  # Convert all answers to string
        # Remove unwanted data
        answer.pop(7)
        answer.pop(0)
        answer.pop(0)
        f_answer = []
        # Formate items in answers
        for item in answer:
            # Convert symbols to full words (w -> white, g -> green etc.)
            if item in convert_answers.keys():
                item = convert_answers[f"{item}"]
            item = item.title()   # Make first letter capital
            item = item.replace('_', ' ')
            f_answer.append(item)
        # Swap column orders
        [f_answer[0], f_answer[1], f_answer[2], f_answer[3], f_answer[4]] = [f_answer[3], f_answer[0], f_answer[1],
                                                                             f_answer[2], f_answer[4]]
        data.append(f_answer)
    return data

def create_table(self, table_data, title='', data_size=10, title_size=12, align_data='L', align_header='L',
                 cell_w='even', x_start='x_default', emphasize_data=None, emphasize_style=None,
    ...
    def get_col_widths():
        ...
        if col_w == 'even':
            ...
            for col in range(len(table_data[0])):  # for every row
                longest = 0
                for row_in_table in range(len(table_data)):
                    cell_value = str(table_data[row_in_table][col])
